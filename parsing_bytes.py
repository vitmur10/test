...
def test_type():
    get_data_from_payload(device_settings) == type(dict)


@pytest.mark.parametrize('a, expencted_result', [(get_data_from_payload(device_settings), test_data),
                                                 (get_data_from_payload(device_settings), field8)])
def test_result(a, expencted_result):
     assert get_data_from_payload(device_settings)
